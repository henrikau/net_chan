...
def calc_bw_req(channel):
    ...
    for k in channel.keys():
        print("{:12s}: {}".format(k, channel[k]))
    dps = int(channel['size']) * 8 * int(channel['freq'])
    print("{}\n\tbps:       {}".format(channel['name'], dps))
    print(    "\tidleSlope: {}".format(dps / 1e6))
...
def parse_file(manifest, verbose=False):
    channels = {}
    block = {}
        ...
        for l in f.readlines():
            ...
            if not found_net_fifo:
                if "struct net_fifo" in line:
                    found_net_fifo = True
                    if verbose:
                        print("Found beginning of struct")
                continue
            else:
                ...
                if '}' in line:
                    ...
                    block = {}
                    if verbose:
                        print("end of block -> {}".format(lvlctr))
...
def main(manifest, txstreams, linkspeed, verbose):
    channels = parse_file(manifest, verbose)
    ...
    print("\tsudo tc qdisc replace dev enp2s0 parent root mqprio num_tc 4 \\")
    ...
    print("\tsudo tc qdisc (add|replace) dev {} parent {} cbs idleslope {} sendslope -{} \\".\
          format("enp2s0", "8003:1", idleSlope_a, sendSlope_a))
    print("\t\thicredit {} locredit -{} offload 1".format(hiCredit_a, loCredit_a))
    print("\tsudo tc qdisc (add|replace) dev {} parent {} cbs idleslope {} sendslope -{} \\".\
          format("enp2s0", "8003:2", idleSlope_b, sendSlope_b))
...
if __name__ == "__main__":
    ...
    main(args.manifest, args.txstreams, args.linkspeed, args.verbose)
